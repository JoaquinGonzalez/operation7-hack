...
void KeyWall()
{
	while(true)
	{
		/*if(GetAsyncKeyState(VK_F1) && 0x8000) n++;
		if(GetAsyncKeyState(VK_F2) && 0x8000) n--;
		if(GetAsyncKeyState(VK_F3) && 0x8000) p++;
		if(GetAsyncKeyState(VK_F4) && 0x8000) p--;
		if(GetAsyncKeyState(VK_F5) && 0x8000) (wallActived ? wallActived = false : wallActived = true);*/
		if(GetAsyncKeyState(VK_END) && 0x8000) menu.Toggle();/*(menuActived ? menuActived = false : menuActived = true);*/
...
{
	if(NumVertices == n && primCount == p &&  wallActived == true && 100 < 0)
	{
